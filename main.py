...
PAIRS   = [
    ...
    # Однонаправленно (Binance покупает → KuCoin продаёт), а не в обе
    # стороны — чтобы не требовалось докидывать капитал на Binance (для
    # двунаправленной схемы ей нужно было бы ~$20.5 вместо текущих ~$14).
