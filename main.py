...
def calc_arb_real(symbol: str, buy_ex: str, buy_ob: Dict, sell_ex: str, sell_ob: Dict,
    ...
    # НОВОЕ 23.08 (по прямому запросу пользователя): АБСОЛЮТНАЯ проверка
    # в долларах, независимая от процентного порога выше. Найдено по 3
    # реальным сделкам 22.08: эрозия исполнения НЕ пропорциональна
    # размеру спреда в процентах (сделка с БОЛЬШИМ % спредом дала БОЛЬШУЮ
    # эрозию в $, не меньшую) — значит, чисто процентный порог решает не
    # ту проблему. Здесь дополнительно требуем, чтобы ожидаемая прибыль
    # сделки в реальных ДОЛЛАРАХ (не только в %) превышала минимум,
    # откалиброванный по факту прошлых потерь ($0.07-0.26, ~$0.15 средняя
    # с запасом) — при слишком маленьком лоте даже честный % спред может
    # давать слишком мало долларов, чтобы пережить типичную эрозию.
    expected_profit_usd = trade_usdt * net / 100
    min_abs = config.get("min_absolute_profit_usd", 0.15)
    if expected_profit_usd < min_abs:
