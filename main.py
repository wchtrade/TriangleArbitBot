...
def calc_arb_real(symbol: str, buy_ex: str, buy_ob: Dict, sell_ex: str, sell_ob: Dict,
    ...
    # учитывающий стоимость ребаланса вообще — а honest_cycle_profit
    # (настоящая, полная оценка с НЕамортизированной стоимостью пересечения
    # спреда при докупке — та же формула, что и в отчёте ПОСЛЕ сделки, см.
    # rebalance_cost_est ниже по коду) считался только ПОСЛЕ исполнения.
    # Найдено по логам 24.08: карточка сама писала "Честная прибыль полного
    # цикла: ~-0.0133 USDT" ДО сделки — и сделка всё равно исполнялась,
    # потому что этот расчёт не участвовал в решении. Причина: порог выше
    # (dynamic_min) амортизирует стоимость ребаланса на sell_reserve_lots
    # (÷3) — предполагая докупку раз в 3 сделки. Но по факту логов докупка
    # происходит почти на КАЖДОЙ сделке — неамортизированная (полная)
    # стоимость ближе к реальности. Теперь ГЕЙТ использует ТУ ЖЕ формулу,
    # что и честная пост-фактум оценка (полностью, не амортизированно) —
    # решение "торговать или нет" принимается на настоящей экономике
    # сделки, а не на оптимистичном приближении.
    naive_profit_usd = trade_usdt * net / 100
    full_rebalance_cost_est = (trade_usdt * (buy_fee + sell_fee)
                                + trade_usdt * config.get("empirical_spread_crossing_pct", 0.34) / 100)
    honest_pretrade_profit_usd = round(naive_profit_usd - full_rebalance_cost_est, 4)
